...
# import requests
import aiohttp  # вместо requests для асинхрона, устанавливается ч/з pip install
import logging
import asyncio
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            ...
            with open(file_path, 'wb') as f:
                f.write(await response.read())  # вместо content() использовал read(), т.к. картинки

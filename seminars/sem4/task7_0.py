...
def threadings_calc():
    ...
    for thread in threads:
        thread.join()
    print(
        f"Функция {threadings_calc.__name__} выполнилась за {time.time() - start_time:.2f} секунд с результатом {sum_arr}")
...
def multy_summ():
    ...
    for process in processes:
        process.join()
    print(
        f"Функция {multy_summ.__name__} выполнилась за {time.time() - start_time:.2f} секунд с результатом {sum_mult.value}")
